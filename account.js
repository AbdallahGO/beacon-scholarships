...
(function () {
  ...
  function render() {
    ...
    const tab = activeTab();
    if (tab !== "profile") teardownCV(); // stop the CV builder's timers/listeners when leaving
    document
  ...
  // ============================ PROFILE (feature 007 — CV builder) ==============
  // The Profile pane is now the CV-style profile builder (specs/007-cv-profile-builder).
  ...
  function renderProfile() {
    ...
    if (!window.BeaconCV || !window.BeaconCV.mount) {
      pane.innerHTML = `<div class="acct-loading">Couldn't load the profile builder — please refresh.</div>`;
      return;
    }
    teardownCV();
    cvTeardown = window.BeaconCV.mount(pane, {
      db,
      user,
      A,
      esc,
